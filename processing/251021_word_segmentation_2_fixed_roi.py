...
for b in range(1,5):
    ...
    for img_path in os.listdir(os.path.join(stimuli_path, b)):
        ...
        word_list, word_spans, space_width, space_height = word_segmentation(os.path.join(stimuli_path, b, img_path), caption, os.path.join(seg_viz_path, img_path))
        ...
        fixed_roi_df = pd.concat([fixed_roi_df, pd.DataFrame({
            ...
            "Tmp": [[int(image_roi_width), int(image_roi_height),
...
# df['Tmp'] = df['Tmp'].apply(lambda x: [int(v/1.65) for v in ast.literal_eval(x)])
